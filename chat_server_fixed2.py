...
# Default room names and their IDs
DEFAULT_ROOMS = {
    'General': 1,
    'Support': 2,
    'Off-Topic': 3
...
def init_chat_rooms():
    ...
    try:
        ...
        for room_name, room_id in DEFAULT_ROOMS.items():
            c.execute('INSERT OR IGNORE INTO chat_rooms (id, name) VALUES (?, ?)', (room_id, room_name))
        conn.commit()
    finally:
        conn.close()
...
async def get_room_id(room_name):
    return DEFAULT_ROOMS.get(room_name)
...
async def main():
    ...
    print("Chat server running on 0.0.0.0:8765")
